        ...
        <table style="font-size: 9pt; width: 100%; margin-top: 5px;">
            <thead>
                <tr>
                    ...
                    <th style="text-align:left;">Name of Report</th>
                    <th style="text-align:left;">Send Date/time</th>
                    <th style="text-align:left;">Done Date/time</th>
            ...
            <tbody>
                @foreach($reports as $clientreport)
                <tr>
                    <td style="text-align:left; width: 45px; {{$loop->last ? 'border-bottom: 1px solid gray' : ''}}">{{$clientreport->report_id}}</td>
                    <td style="text-align:left; {{$loop->last ? 'border-bottom: 1px solid gray' : ''}}">{{$clientreport->report_name}}</td>
                    <td style="text-align:left; {{$loop->last ? 'border-bottom: 1px solid gray' : ''}}">{{date("F j, Y, g:i a", strtotime($clientreport->send_datetime))}}</td>
                    <td style="text-align:left; {{$loop->last ? 'border-bottom: 1px solid gray' : ''}}">{{$clientreport->done_datetime != null ? date("F j, Y, g:i a", strtotime($clientreport->done_datetime)) : ""}}</td>
                    <td style="text-align:left; {{$loop->last ? 'border-bottom: 1px solid gray' : ''}}">{{$clientreport->accept_user}}</td>
                    <td style="text-align:left; {{$loop->last ? 'border-bottom: 1px solid gray' : ''}}">{{$clientreport->status_name}}</td>
                    <td style="text-align:left; {{$loop->last ? 'border-bottom: 1px solid gray' : ''}}">{{$clientreport->situation_name}}</td>
                </tr>
                @endforeach
              </tr>
