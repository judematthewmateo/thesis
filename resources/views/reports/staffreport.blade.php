...
<html>
    ...
    <body>
        ...
        <table style="font-size: 9pt; width: 100%; margin-top: 5px;">
            ...
            <tbody>
                @foreach($reports as $staffreport)
                <tr>
                    <td style="text-align:left; width: 45px; {{$loop->last ? 'border-bottom: 1px solid gray' : ''}}">{{$staffreport->report_id}}</td>
                    <td style="text-align:left; {{$loop->last ? 'border-bottom: 1px solid gray' : ''}}">{{$staffreport->report_name}}</td>
                    <td style="text-align:left; {{$loop->last ? 'border-bottom: 1px solid gray' : ''}}">{{date("F j, Y, g:i a", strtotime($staffreport->send_datetime))}}</td>
                    <td style="text-align:left; {{$loop->last ? 'border-bottom: 1px solid gray' : ''}}">{{date("F j, Y, g:i a", strtotime($staffreport->done_datetime))}}</td>
                    <td style="text-align:left; {{$loop->last ? 'border-bottom: 1px solid gray' : ''}}">{{$staffreport->accept_user}}</td>
                    <td style="text-align:left; {{$loop->last ? 'border-bottom: 1px solid gray' : ''}}">{{$staffreport->status_name}}</td>
                    <td style="text-align:left; {{$loop->last ? 'border-bottom: 1px solid gray' : ''}}">{{$staffreport->situation_name}}</td>
                </tr>
                @endforeach
              </tr>
