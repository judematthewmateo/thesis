...
export default {
  methods: {
    ...
    createEntity(entity, isModal, entity_table, is_tab = false) {
      ...
      this.$http
        .post("api/" + entity, this.forms[entity].fields, {
          headers: {
    ...
    createOptionsEntity(entity, isModal, entity_table, from_entity, entity_id) {
      ...
      this.$http
        ...
        .then(response => {
          ...
          setTimeout(
            function() {
              this.forms[from_entity].fields[entity_id] =
    ...
    updateEntity(entity, entity_id, isModal, row, is_tab = false) {
      ...
      this.$http
        .put(
          "api/" + entity + "/" + this.forms[entity].fields[entity_id],
          this.forms[entity].fields,
          {
            headers: {
              Authorization: "Bearer " + localStorage.getItem("token")
            }
    ...
    deleteEntity(entity, entity_id, isModal, entity_table, primary_key) {
      ...
      this.$http
        ...
        .then(response => {
          ...
          const index = this.tables[entity_table].items.findIndex(
            item => item[primary_key] === response.data.data[primary_key]
          );
    ...
    filterTableList(entity, filter1, filter2) {
      this.$http
        .get("/api/" + entity + "/" + filter1 + "/" + filter2, {
          headers: {
            Authorization: "Bearer " + localStorage.getItem("token")
          }
        })
    ...
    checkRights(right_code) {
      ...
      var row = this.$store.state.rights.filter(
        right => right.right_code == right_code
      );
      if (row.length > 0) {
        right = true;
      }
    ...
    checkParentRights(right_code) {
      ...
      right_code.forEach(rc => {
        var row = this.$store.state.rights.filter(
          right => right.right_code.split("-")[0] == rc
        );
        length += row.length;
      });
      if (length > 0) {
